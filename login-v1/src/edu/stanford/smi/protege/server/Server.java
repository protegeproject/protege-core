...
public class Server extends UnicastRemoteObject implements RemoteServer {
    ...
    private Map<RemoteSession, ServerProject> sessionToProjectMap = new HashMap<RemoteSession, ServerProject>();
    ...
    public LoginToken openSession(String username, String userIpAddress, String password) {
        LoginToken token = null;
        if (isValid(username, password)) {
            token = new LoginToken(username, userIpAddress);
    ...
    public void closeSession(RemoteSession session) {
        sessionToProjectMap.remove(session);
    }
    
    public boolean isActive(RemoteSession session) {
        return sessionToProjectMap.get(session) != null;
    }

    public RemoteServerProject openProject(String projectName, LoginToken token) 
    throws ServerSessionLost {
        ...
        if (p != null) {
            ...
            recordConnection(token, serverProject);
        }
        return serverProject;
    }

    private void recordConnection(LoginToken token, ServerProject project) 
    throws ServerSessionLost {
        RemoteSession session = new Session(token);
        sessionToProjectMap.put(session, project);
        project.register(session);
        Log.getLogger().info("starting session " + session);
    }
    ...
    public Collection<String> getAvailableProjectNames(LoginToken token) {
    ...
    public Collection getCurrentSessions(String projectName, LoginToken token) {
    ...
    public Collection<RemoteSession> getCurrentSessions(RemoteServerProject project) {
        ...
        for (Map.Entry<RemoteSession, ServerProject> entry : sessionToProjectMap.entrySet()) {
            ServerProject openProject = entry.getValue();
            if (openProject.equals(project)) {
