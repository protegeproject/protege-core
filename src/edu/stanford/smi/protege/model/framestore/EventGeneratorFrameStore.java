...
public class EventGeneratorFrameStore extends ModificationFrameStore {
    ...
    /*
     * TODO We should actually generate some subclass removed events but not
     * all. The problem is that it is problematic to generate subclass removed
     * events on superclasses that have also been deleted. Receivers of these
     * events are going to get the "changed" event first and process it when in
     * fact the superclass has also been deleted but they haven't gotten that
     * event yet. This will lead them to make calls with deleted frames have
     * have trouble.
     * 
     * It would seem that the only way around this is to change the definition
     * of "delete class" so that it only works on leaf classes (just as it only
     * works on classes with no instances at the moment). This would probably be
     * cleaner and would simplify the interface. The differences could be
     * handled at either the KB or the FrameStoreManager level.
     * 
     * For now though we hack it...
     */
    /*
     * private void generateDeleteSubclassEvents(Cls cls) { Iterator i =
     * SimpleFrameStore.getClsesToBeDeleted(cls, this).iterator(); while
     * (i.hasNext()) { Cls subclass = (Cls) i.next(); if (!subclass.equals(cls)) {
    ...
    private void generateOwnSlotValuesChangedEvent(Frame frame, Slot slot) {
        int count = getDirectOwnSlotValuesCount(frame, slot);
        Collection oldValues = new ArrayList();
        if (count < MAX_VALUES) {
            oldValues.addAll(getDirectOwnSlotValues(frame, slot));
        }
