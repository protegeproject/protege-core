...
public class InMemoryFrameDb implements NarrowFrameStore {
    ...
    private void safeRemove(Collection c, Object v) {
        if (c != null) {
            if (c.size() > 100 && c instanceof List) {
                // Log.trace("Big collection: " + c.size() + " " + c.getClass(), this, "safeRemove", v);
            }
            c.remove(v);
        }
    }
    ...
    public void moveValue(Frame frame, Slot slot, Facet facet, boolean isTemplate, int from, int to) {
        ...
        if (to <= values.size()) {
            ...
            Log.error("Invalid target index", this, "", frame, slot, facet, new Integer(from), new Integer(to));
        }
    }
    ...
    private boolean _isInferred(Slot slot, Facet facet, boolean isTemplate) {
        ...
        if (isInferred) {
            ...
            isInferred =
                id.equals(Model.SlotID.DIRECT_SUBCLASSES)
    ...
    private void removeFrameAsReferenceElement(Frame frame) {
        /*
        Iterator i = new ArrayList(_referenceToValuesMap.keySet()).iterator();
        while (i.hasNext()) {
            Reference reference = (Reference) i.next();
            if (usesFrame(reference, frame)) {
    ...
    /*
    private static boolean usesFrame(Reference reference, Frame frame) {
        return equals(frame, reference.getFrame())
            || equals(frame, reference.getSlot())
            || equals(frame, reference.getFacet());
    }
    */
    ...
    private static boolean matches(Reference reference, Slot slot, Facet facet, boolean isTemplate) {
        boolean matches =
            equals(slot, reference.getSlot())
                && equals(facet, reference.getFacet())
    ...
    public Set getFramesWithAnyValue(Slot slot, Facet facet, boolean isTemplate) {
        Set frames = new HashSet();
        /*
        Iterator i = _referenceToValuesMap.keySet().iterator();
        while (i.hasNext()) {
            Reference ref = (Reference) i.next();
            if (matches(ref, slot, facet, isTemplate)) {
                frames.add(ref.getFrame());
    ...
    /*
    private void replaceFrameInFrameToContainingReferencesMap(Frame frame) {
        Collection containingReferences = (Collection) _frameToContainingReferencesMap.get(frame);
        Iterator i = containingReferences.iterator();
        while (i.hasNext()) {
    ...
    */
    ...
    // We can't mysteriously produce any frames down here.  If we don't have one by now we aren't going to get one.
