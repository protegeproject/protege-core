...
public class BundleHelper {
    ...
    public BundleHelper(String bundleName, Class clas) {
        try {
            ...
            // Locale locale = ApplicationProperties.getLocale(); // Locale.getDefault()
            // Locale.setDefault(locale);
            resourceBundle = ResourceBundle.getBundle(bundleName, locale, loader);
            // Log.getLogger().finest("default locale=" + locale);
        } catch (MissingResourceException e) {
            Log.warning("missing bundle", this, "BundleHelper", bundleName, clas);
        }
    }
    ...
    private String internalGetText(ResourceKey key) {
        ...
        if (resourceBundle == null) {
            Log.error("no resource bundle", BundleHelper.class, "getText", key);
