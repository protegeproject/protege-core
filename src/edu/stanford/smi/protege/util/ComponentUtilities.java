...
public class ComponentUtilities {
    ...
    public static void extendSelection(JTree tree, Object userObject) {
        ...
        if (index == -1) {
            Log.warning("object not found", ComponentUtilities.class,
                    "extendSelection", tree, userObject);
        } else {
    ...
    public static TreePath getTreePath(JTree tree, Collection objectPath) {
        ...
        while (i.hasNext()) {
            ...
            if (node == null) {
                String text = "Child node not found: " + userObject;
                Log.trace(text, ComponentUtilities.class, "getTreePath", tree,
                        objectPath);
                return null;
            }
            nodePath.add(node);
