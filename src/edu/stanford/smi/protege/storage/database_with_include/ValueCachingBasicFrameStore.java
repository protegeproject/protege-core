...
/*
 * class Fsft { private Frame frame; private Slot slot; private Facet facet;
 * private boolean isTemplate; private int hashCode;
 * 
 * Fsft(Frame frame, Slot slot, Facet facet, boolean isTemplate) { set(frame,
 * slot, facet, isTemplate); } Fsft() { }
 * 
 * void set(Frame frame, Slot slot, Facet facet, boolean isTemplate) {
 * this.frame = frame; this.slot = slot; this.facet = facet; this.isTemplate =
 * isTemplate; hashCode = HashUtils.getHash(frame, slot, facet, isTemplate); }
 * 
 * boolean contains(Frame frame) { return frame.equals(this.frame) ||
 * frame.equals(this.slot) || frame.equals(this.facet); }
 * 
 * public int hashCode() { return hashCode; }
 * 
 * public boolean equals(Object o) { boolean result = false; if (o instanceof
 * Fsft) { Fsft rhs = (Fsft) o; result = equals(facet, rhs.facet) &&
 * equals(slot, rhs.slot) && equals(facet, rhs.facet) && isTemplate ==
 * rhs.isTemplate; } return result; }
 * 
 * private static boolean equals(Object o1, Object o2) { return
 * SystemUtilities.equals(o1, o2); } }
 * 
 * public class ValueCachingBasicFrameStore implements BasicFrameStore { private
 * DatabaseFrameDb _delegate; private final Fsft _lookupFsft = new Fsft();
 * private CacheMap _fsftToValuesMap = new CacheMap();
 * 
 * public void close() { _delegate.close(); _delegate = null; _fsftToValuesMap =
 * null; }
 * 
 * public ValueCachingBasicFrameStore(DatabaseFrameDb delegate) { _delegate =
 * delegate; }
 * 
 * public BasicFrameStore getDelegate() { return _delegate; }
 * 
 * public FrameID generateFrameID() { return _delegate.generateFrameID(); }
 * public Frame getFrame(FrameID id) { return _delegate.getFrame(id); }
 * 
 * private List lookup(Frame frame, Slot slot, Facet facet, boolean isTemplate) {
 * _lookupFsft.set(frame, slot, facet, isTemplate); return (List)
 * _fsftToValuesMap.get(_lookupFsft); }
 * 
 * public List getValues(Frame frame, Slot slot, Facet facet, boolean
 * isTemplate) { List values = lookup(frame, slot, facet, isTemplate); if
 * (values == null) { values = _delegate.getValues(frame, slot, facet,
 * isTemplate); setCacheValues(frame, slot, facet, isTemplate, values); } if
 * (values == null) { values = Collections.EMPTY_LIST; } return values; }
 * 
 * public void setValues(Frame frame, Slot slot, Facet facet, boolean
 * isTemplate, Collection values) { _delegate.setValues(frame, slot, facet,
 * isTemplate, values); setCacheValues(frame, slot, facet, isTemplate, values); }
 * 
 * public void addValues(Frame frame, Slot slot, Facet facet, boolean
 * isTemplate, Collection values) { _delegate.addValues(frame, slot, facet,
 * isTemplate, values); addCacheValues(frame, slot, facet, isTemplate, values); }
 * 
 * public void removeValue(Frame frame, Slot slot, Facet facet, boolean
 * isTemplate, Object value) { _delegate.removeValue(frame, slot, facet,
 * isTemplate, value); removeCacheValue(frame, slot, facet, isTemplate, value); }
 * 
 * public void moveValue(Frame frame, Slot slot, Facet facet, boolean
 * isTemplate, int from, int to) { // _delegate.moveValue(frame, slot, facet,
 * isTemplate, from, to); List values = new ArrayList(getValues(frame, slot,
 * facet, isTemplate)); Object o = values.remove(from); values.add(to, o);
 * _delegate.setValues(frame, slot, facet, isTemplate, values);
 * 
 * moveCacheValue(frame, slot, facet, isTemplate, from, to); }
 * 
 * public List getFrames(Slot slot, Facet facet, boolean isTemplate, Object
 * value) { return _delegate.getFrames(slot, facet, isTemplate, value); }
 * 
 ...
 * 
 * public List getMatchingFrames(Slot slot, Facet facet, boolean isTemplate,
 * String value, int maxMatches) { return _delegate.getMatchingFrames(slot,
 * facet, isTemplate, value, maxMatches); }
 * 
 * public List getReferences(Object value) { return
 * _delegate.getReferences(value); }
 * 
 * public List getMatchingReferences(String value, int maxMatches) { return
 * _delegate.getMatchingReferences(value, maxMatches); }
 * 
 * public void deleteFrame(Frame frame) { deleteCacheFrame(frame);
 * _delegate.deleteFrame(frame); }
 * 
 * public List executeQuery(Query query) { return _delegate.executeQuery(query); }
 * 
 * private void setCacheValues(Frame frame, Slot slot, Facet facet, boolean
 * isTemplate, Collection values) { Fsft fsft = new Fsft(frame, slot, facet,
 * isTemplate); _fsftToValuesMap.put(fsft, new ArrayList(values)); }
 * 
 * private void addCacheValues(Frame frame, Slot slot, Facet facet, boolean
 * isTemplate, Collection values) { List currentValues = lookup(frame, slot,
 * facet, isTemplate); if (currentValues != null) {
 * currentValues.addAll(values); } }
 * 
 * private void removeCacheValue(Frame frame, Slot slot, Facet facet, boolean
 * isTemplate, Object value) { List list = lookup(frame, slot, facet,
 * isTemplate); if (list != null) { list.remove(value); } } private void
 * moveCacheValue(Frame frame, Slot slot, Facet facet, boolean isTemplate, int
 * from, int to) { List list = lookup(frame, slot, facet, isTemplate); if (list !=
 * null) { Object value = list.remove(from); list.add(to, value); } } private
 * void deleteCacheFrame(Frame frame) { Iterator i =
 * _fsftToValuesMap.getKeys().iterator(); while (i.hasNext()) { Fsft fsft =
 * (Fsft) i.next(); if (fsft.contains(frame)) { _fsftToValuesMap.remove(fsft); }
 * else { List values = (List) _fsftToValuesMap.get(fsft); values.remove(frame); } } }
 * 
 * public boolean beginTransaction() { return getDelegate().beginTransaction(); }
 * 
 * public boolean commitTransaction() { boolean committed =
 * getDelegate().commitTransaction(); if (!committed) { clearCache(); } return
 * committed; }
 * 
 * public boolean rollbackTransaction() { boolean rolledback =
 * getDelegate().rollbackTransaction(); if (rolledback) { clearCache(); } return
 * rolledback; }
 * 
 * public void clearCache() { _fsftToValuesMap.clear(); }
 * 
 * public void replaceFrame(Frame frame) { clearCache();
 * getDelegate().replaceFrame(frame); }
 * 
 * public int getFrameCount() { return getDelegate().getFrameCount(); } public
 * int getClsCount() { return getDelegate().getClsCount(); } public int
 * getSlotCount() { return getDelegate().getSlotCount(); } public int
 * getFacetCount() { return getDelegate().getFacetCount(); } public int
 * getSimpleInstanceCount() { return getDelegate().getSimpleInstanceCount(); } }
 */
// /*
public class ValueCachingBasicFrameStore implements NarrowFrameStore {
    ...
    private boolean isSpecial(Slot slot, Facet facet, boolean isTemplate) {
        return facet == null
                && !isTemplate
                && (equals(slot, Model.SlotID.DIRECT_INSTANCES) || equals(slot,
    ...
    private List loadSpecialValuesIntoCache(Map map, Frame frame, Slot slot, Facet facet,
            boolean isTemplate) {
    ...
    // I don't really want to cache the return value here but I don't want to
    // have to keep going back to the db
    // if it is small. Thus is the value is small we load the frame, otherwise
    // we just go to the db on every call
    ...
    private void setCacheValues(Frame frame, Slot slot, Facet facet, boolean isTemplate,
            Collection values) {
        Map sftToValuesMap = lookup(frame);
        if (sftToValuesMap != null) {
    ...
    private void addCacheValues(Frame frame, Slot slot, Facet facet, boolean isTemplate,
            Collection values) {
        Map sftToValuesMap = lookup(frame);
        if (sftToValuesMap != null) {
    ...
    private void removeCacheValue(Frame frame, Slot slot, Facet facet, boolean isTemplate,
            Object value) {
    ...
    private void moveCacheValue(Frame frame, Slot slot, Facet facet, boolean isTemplate, int from,
            int to) {
        List list = lookup(frame, slot, facet, isTemplate);
        if (list != null) {
