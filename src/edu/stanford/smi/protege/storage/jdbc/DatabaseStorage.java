...
import edu.stanford.smi.protege.storage.jdbc.framedb.Record;
import edu.stanford.smi.protege.util.*;

/**
 * Implementation of storage interfaces for database. This implementation delegates in memory behavior to FrameDBStorage
 * and additionally sends commands along to its DatabaseManager (from whence they make it to the real db).
 * 
...
public class DatabaseStorage implements Storage, DatabaseConstants {
    ...
    public DatabaseStorage(OldJdbcDefaultKnowledgeBase kb, DatabaseManager manager,
    ...
    private FrameID getDBDirectType(FrameID id) {
        FrameID result;
        try {
            Collection frameIDs = _databaseManager.getValues(id, Model.SlotID.DIRECT_TYPES, null,
                    false);
    ...
    private FrameID getDBFrameID(String name) {
        ...
        try {
            ...
            if (ids.size() > 1) {
                Log.getLogger().warning(ids.size() + " frames with the same name: " + name);
            }
    ...
    private String getDBName(FrameID id) {
        ...
        try {
            ...
            if (values.size() != 1) {
                Log.getLogger().warning("problem with frame name: " + id);
            }
    ...
    public int getDBValueCount(Frame frame, Slot slot, Facet facet, boolean isTemplate) {
        int count;
        try {
            count = _databaseManager.getValueCount(getID(frame), getID(slot), getID(facet),
                    isTemplate);
        } catch (SQLException e) {
            Log.getLogger().warning(e.toString());
            count = 0;
    ...
    public Collection getFrames() {
        ...
        try {
            ...
            while (i.hasNext()) {
                ...
                if (frame == null) {
                    Log.getLogger().warning("no frame for id: " + id);
                } else {
                    frames.add(frame);
                }
            }
    ...
    public Collection getFramesWithValue(Slot slot, Facet facet, boolean isTemplate, Object value) {
        ...
        try {
            Collection frameIDs = _databaseManager.getFramesWithValue(slot, facet, isTemplate,
                    value);
    ...
    public Collection getMatchingFrames(Slot slot, Facet facet, boolean isTemplate,
            String matchValue, int maxMatches) {
        Collection matchedFrames;
        ...
        try {
            ...
            Collection frameIDs = _databaseManager.getMatchingFrameIDs(slotID, facetID, isTemplate,
                    matchValue, maxMatches);
            matchedFrames = new ArrayList(frameIDs.size());
            Iterator i = frameIDs.iterator();
            while (i.hasNext()) {
                FrameID id = (FrameID) i.next();
    ...
    private Frame loadFrameFromDB(FrameID id, String name) {
        ...
        case TYPE_SIMPLE_INSTANCE:
            ...
            break;
        default:
            Log.getLogger().warning("bad frame type: " + frameType);
    ...
    private boolean needsUpdate(FrameID slot) {
        return slot == Model.SlotID.DIRECT_INSTANCES || slot == Model.SlotID.DIRECT_SUBCLASSES
    ...
    public void removeValues(Slot slot, Facet facet, boolean isTemplate, Cls cls) {
        ...
        try {
            if (cls == null) {
                ...
                Iterator i = _databaseManager.getFramesWithAnyValue(slot, facet, isTemplate)
                        .iterator();
                while (i.hasNext()) {
    ...
    private void updateDB(Frame frame, Slot slot, Facet facet, boolean isTemplate)
            throws SQLException {
        Collection values = _delegate.getValues(frame, slot, facet, isTemplate);

        /*
         * The following unfortunate logic keeps us from recording such things as that :CLASS is a child of :THING. We
         * don't want the system class hierarchy stored in a database because it is still evolving. Nevertheless we have
         * to record that a user defined class is a subclass of :THING. Thus we need to store some of the subclasses of
    ...
    private void updateDelegate() {
        ...
        while (i.hasNext()) {
            ...
            if (needsUpdate(slot)) {
                ...
                if (!values.isEmpty()) {
                    // Log.trace("adding " + values.get(0) + " ( " + values.size() + ") to " + frame + " " + slot + " "
                    // + facet, this, "updateDelegate");
