...
public class RemoteProjectManager {
    ...
    public Project getRemoteProject() {
        ...
        if (rval == ModalDialog.OPTION_OK) {
            ...
            token = panel.getSession();
            if (server != null && token != null) {
                project = getServerProject(mainPanel, server, token);
            }
        }
        return project;
    }

    public Project getServerProject(JComponent parent, RemoteServer server, LoginToken session) {
        Project project = null;
    ...
    public Project getProject(String serverName, 
        ...
        try {
            ...
            if (server != null) {
                LoginToken token = server.openSession(username, SystemUtilities.getMachineIpAddress(), password);
                if (token != null) {
                    RemoteServerProject serverProject = server.openProject(projectName, token);
                    if (serverProject != null) {
                        p = RemoteClientProject.createProject(server, serverProject, token, pollForEvents);
                    }
                }
            }
        } catch (Exception e) {
