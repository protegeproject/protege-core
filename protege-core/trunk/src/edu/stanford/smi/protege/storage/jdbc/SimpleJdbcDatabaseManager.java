 ...
 * Everything is a slot value or a facet value.  The schema is totally useless for someone trying to write reasonable
 * SQL to get information out of their knowledge base.  Such a backend could certainly be developed.  
...
public class SimpleJdbcDatabaseManager implements DatabaseManager {
    ...
    */
    ...
    public void addValue(Frame frame, Slot slot, Facet facet, boolean isTemplate, Object value) throws SQLException {
        if (value == null) {
            ...
            String text =
                "null value in SimpleJdbcDatabaseManager.addValue("
                    + frame
                    + ", "
                    + slot
                    + ", "
    ...
    private Object convertValue(String s, int type) {
        ...
        switch (type) {
            ...
            case TYPE_SLOT :
                ...
                value = (val < FrameID.INITIAL_USER_FRAME_ID) ? FrameID.createSystem(val) : FrameID.createLocal(val);
                break;
            default :
                // Assert.fail("bad type: " + type);
                Log.stack("bad type", this, s, new Integer(type));
                value = null;
                break;
        }
        return value;
    ...
    */
    ...
    public void createTable() throws SQLException {
        ...
        try {
            ...
            Log.warning("failed: " + e.getMessage(), this, "createTable");
        } finally {
            stmt.close();
        }
    }
    ...
    */
    ...
    private void dropTable() throws SQLException {
        ...
        try {
            ...
            Log.trace("failed: " + e.getMessage(), this, "dropTable");
        }
        stmt.close();
    }
    */
    ...
    public boolean endTransaction(boolean doCommit) {
        ...
        if (_canDisableAutoCommit) {
            try {
                ...
                _canDisableAutoCommit = false;
                Log.warning("transaction failed, disabling transactions", this, "endTransaction");
            }
        }
        return result;
    }
    ...
    public Collection getMatchingFrameIDs(
        FrameID slot,
        FrameID facet,
        boolean isTemplate,
        String value,
    ...
    */
    ...
    public List getValues(FrameID frame, FrameID slot, FrameID facet, boolean isTemplate) throws SQLException {
        ...
        List values = (List) dbReferenceToValuesMap.get(new DBReference(frame, slot, facet, isTemplate));
        if (values == null) {
    ...
    private void cacheInsert(Map map, FrameID frame, FrameID slot, FrameID facet, boolean isTemplate, Object value) {
    ...
    */
    ...
    public void removeFramesWithValue(Slot slot, Facet facet, boolean isTemplate) throws SQLException {
        ...
        String command =
            "DELETE FROM " + _tableName + " WHERE " + SLOT + "= ? AND " + FACET + "= ? AND " + IS_TEMPLATE + "= ?";
        PreparedStatement stmt = _connection.prepareStatement(command);
        setSlotParam(stmt, 1, slot);
        setFacetParam(stmt, 2, facet);
        setIsTemplateParam(stmt, 3, isTemplate);
    ...
    public void removeSingleValue(Frame frame, Slot slot, Facet facet, boolean isTemplate, Object value)
        throws SQLException {
        trace("removeSingleValue", frame, slot, facet, value);
        String command = "DELETE FROM " + _tableName;
    ...
    */
    ...
    public void saveKnowledgeBase(KnowledgeBase kb) throws SQLException {
        ...
        _directTypeSlot = kb.getSlot(Model.Slot.DIRECT_TYPES);
