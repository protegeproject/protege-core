...
public class Project {
    ...
    private Map _includedBrowserSlotPatterns = new HashMap(); // <Cls, BrowserSlotPattern>
    private Map _directBrowserSlotPatterns = new HashMap(); // <Cls, BrowserSlotPattern>
    ...
    */
    ...
    protected InstanceDisplay createInstanceDisplay(Project project, boolean showHeader, boolean showHeaderLabel) {
        ...
        if (_instanceDisplayClass != null) {
            instanceDisplay = createInstanceDisplaySubclass(_instanceDisplayClass, project, showHeader, showHeaderLabel);
        } else {
    ...
    private static InstanceDisplay createInstanceDisplaySubclass(Class clas, Project project, boolean showHeader, boolean showHeaderLabel) {
        ...
        Object[] args = new Object[] {project, new Boolean(showHeader), new Boolean(showHeaderLabel)};
        try {
	        Constructor constructor = clas.getConstructor(classes);
	        instanceDisplay = (InstanceDisplay) constructor.newInstance(args);
        } catch (Exception e) {
            Log.exception(e, Project.class, "createInstanceDisplaySubclass", clas, project);
        }
        return instanceDisplay;
}
    ...
    public ClsWidget getDesignTimeClsWidget(Cls cls) {
        ...
        if (widget == null) {
            ...
            // Widgets have to go into a cache because we only want one design time widget per widget descriptor.
            // Otherwise when a dt widget writes to the descriptor (to delete the current property list, for example)
    ...
    /**
     * @return the URIs of all included projects, including indirectly included ones
     */
    public Collection getIncludedProjects() {
        return Collections.unmodifiableCollection(_cachedIncludedProjectURIs);
    }
     ...
     * @return A collection of strings which are the absolute file paths directly included Projects that are files.  If
     * an directly included Project is not a file then IllegalArgumentException is thrown 
     */
    ...
    private static Reader getProjectClsesReader() {
        ...
        if (reader == null) {
            Log.error("Unable to read system ontology", Project.class, "getProjectClsesReader");
        }
        return reader;
    }
    ...
    protected static Instance getProjectInstance(KnowledgeBase kb) {
        ...
        if (result == null) {
            Log.error("no project instance", Project.class, "getProjectInstance", kb);
        }
        return result;
    }
    ...
    private static Reader getProjectInstancesReader(URI uri, KnowledgeBaseFactory factory, Collection errors) {
        ...
        if (reader == null && factory != null) {
            ...
            if (path != null) {
                ...
                if (reader == null) {
                    Log.error("Unable to read factory project: " + path, Project.class, "getProjectInstancesReader");
                }
            }
        }
        if (reader == null) {
            ...
            if (reader == null) {
                Log.error("Unable to read system instances", Project.class, "getProjectInstancesReader");
            }
        }
        return reader;
    }
        ...
        if (_tabWidgetDescriptors == null) {
            ...
            while (i.hasNext()) {
                ...
                if (PluginUtilities.isLoadableClass(d.getWidgetClassName())) {
                    ...
                    // String text = "removing reference to missing tab: " + d.getWidgetClassName();
    ...
    public void includeProject(URI uri, boolean doLoad, Collection errors) {
        if (doLoad) {
            loadIncludedProject(uri, errors);
        }
    ...
    private boolean isAlreadyIncluded(URI uri) {
        boolean isAlreadyIncluded = _uri != null && equalsURI(uri, _uri);
        if (!isAlreadyIncluded) {
            Iterator i = _cachedIncludedProjectURIs.iterator();
            while (i.hasNext() && !isAlreadyIncluded) {
                URI includedURI = (URI) i.next();
                isAlreadyIncluded = equalsURI(uri, includedURI);
            }
        }
        return isAlreadyIncluded;
    }

    private static boolean equalsURI(URI uri1, URI uri2) {
        return uri1.toString().equalsIgnoreCase(uri2.toString());
    }
    ...
    private void loadIncludedProject(URI includedURI, Collection errors) {
        includedURI = includedURI.normalize();
        if (!isAlreadyIncluded(includedURI)) {
            // Log.enter(this, "loadIncludedProject", includedURI);
            _cachedIncludedProjectURIs.add(includedURI);
            KnowledgeBase kb = loadProjectKB(includedURI, null, errors);
            if (kb != null) { // && errors.size() == 0) {
                // This business allows included projects (and their domain kbs) to load from other subdirectories
    ...
    protected static KnowledgeBase loadProjectKB(URI uri, KnowledgeBaseFactory factory, Collection errors) {
        ...
        try {
            ...
            if (instancesReader == null) {
                 ...
                 * there are lot of unreferenced instances (from temporary widget descriptors) that have to be kept in 
                 * memory.
    ...
    /*
     * This method doesn't merge the client information from included projects.  It is unclear how to even do this.  The
     * included project client information shouldn't be written out into the including project file.  This means that
     * not only must the information from multiple project be merged there must be a way to separate it back out on save.
     * I'm not really sure how to do this.
     * 
     * For the moment the client information is just the information from the last loaded (outermost) project.
     */
        ...
        while (i.hasNext()) {
            ...
            if (d == null) {
                ...
                if (cls == null) {
                    ...
                    // be careful not to overwrite widgets on an "include" command.
                    WidgetDescriptor existingDescriptor = (WidgetDescriptor) _activeClsWidgetDescriptors.get(cls);
    ...
    public void removeIncludedProjectReferences() {
        ...
        _cachedIncludedProjectURIs.clear();
    ...
    private static void removeUnreferencedInstances(KnowledgeBase kb) {
        ...
        while (i.hasNext()) {
            Instance instance = (Instance) i.next();
            // Log.trace("found unreachable instance: " + instance, Project.class, "removeUnreferencedInstances");
    ...
    private void saveDefaultMetaclasses() {
        ...
        setProjectSlotValue(SLOT_DEFAULT_FACET_METACLASS, getName(_domainKB.getDefaultFacetMetaCls()));
    }
    ...
    private void saveHiddenFrameFlags() {
        ...
        while (i.hasNext()) {
            ...
            if (!_includedHiddenFrames.contains(frame)) {
                ...
                if (name == null) {
                    Log.warning("Ignoring nameless frame " + frame, this, "saveHiddenFrameFlags");
                } else {
    ...
    /*
     * Deliberately reduce the visibility of this method.  Instead people should call
     * Cls.setDirectBrowserSlot or KnowledgeBase.setDirectBrowserSlot 
     */
    ...
    /*
     * Direct included projects are stored as paths relative to the project uri.  When the project uri changes 
     * these paths need to be recalculated relative to the new project uri.
     */
    ...
    /** 
     * Returns previously stored "arbitrary" client information.  This information is view specific and differs from
     * the client information available on the {@link KnowledgeBase} class.  Examples of this type of information is the 
     * positioning of a yellow sticky on a form, or the positioning of a node in the diagram widget. <br>
     * <br>
     * The "client information" attached to a project is persistent.  In order for the persistence mechanism to work
     * correctly the keys and values stored as client information must either be a domain knowledge base frame or be 
     * convertible to a string.<br>
     * <br>
     * Typical values are things such as Strings, Rectangles, and Maps of other objects.  The other objects included
     * in a map must themselves be either Frames or must be convertable to and from a string.  Being convertable to and
     * from a String means:<br>
     * (1) The object.toString() method must return the state of the object.<br>
     * (2) There class must has a constructor that takes a single string argument.<br>  
    ...
    /** Set persistent client information.  See {@link #getClientInformation(Object)} for more information about "client
     * information".
     */
