...
public class FrameStoreManager {
    ...
    private void dumpFrameStoreChain(String text) {
        ...
        System.out.println("Frame Store Chain at " + text);
        Set visitedFrameStores = new HashSet();
        for (FrameStore fs = headFrameStore; !fs.equals(terminalFrameStore); fs = fs.getDelegate()) {
            boolean changed = visitedFrameStores.add(fs);
            Assert.true("loop in framestores", changed);
    ...
    */
    ...
    private FrameStore create(Class clas) {
        ...
        try {
            ...
            Log.exception(e, this, "create", clas);
        }
        return frameStore;
    }
    ...
    private boolean enable(FrameStore frameStore) {
        ...
        boolean wasEnabled =
            (preceeding == null) ? headFrameStore.equals(frameStore) : preceeding.getDelegate().equals(frameStore);
        if (!wasEnabled) {
