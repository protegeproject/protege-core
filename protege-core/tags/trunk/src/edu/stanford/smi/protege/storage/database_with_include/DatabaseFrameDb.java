...
public class DatabaseFrameDb implements NarrowFrameStore {
    ...
    private void initializeTracing() {
        _isTracingUpdate = ApplicationProperties.getBooleanProperty("database.tracing.update", false);
    ...
    /* Using the database metadata to decide if a table exists turns out to be unreliable.
     * Instead we just do a select of something fast and make sure that the table isn't completely
     * empty.  If this succeeds the table is assumed to be ok.
     */
    ...
    private void createIndices() throws SQLException {
        ...
        /* VALUE_INDEX is included in this index solely for its value as a side effect.  It keeps the values ordered 
         * by the VALUE_INDEX so that 
         * the ORDER_BY clause in the getValues SELECT statement does not cost anything.  Otherwise this clause
         * has a huge performance impact.  We are essentially trading disk space and INSERT time for a SELECT
         * optimization.
         */
     ...
     * Thus for these databases we lower case the string before matching and match on a LOWER function on the column.  
     * To work efficiently this requires an index on the LOWER of the column.  For some bizarre reason on Oracle doing
     * an index on the function of a column requires that a user have an obscure addition privelege on the database.
    ...
    /* This call is implemented as multiple SQL calls rather than a single one.  This is because
     * surprisingly, it turns out to be much faster to execute them separately.  The MySQL optimizer
     * (and perhaps others) has trouble using the indices in the presence of an OR.
     */
    ...
    public void moveValue(Frame frame, Slot slot, Facet facet, boolean isTemplate, int from, int index) {
        checkModifiability();
        // this implementation could probably be optimized but it probably isn't worth it
    ...
    /*
     * We need to return the value and test for a "real match" because MySQL will match in a case insensitive way.  
     * This really messes up our frame lookup by name because different types of frames often only differ by the case
     * of their names.
     */
    ...
    public void setValues(Frame frame, Slot slot, Facet facet, boolean isTemplate, Collection values) {
        // Log.enter(this, "setValues", frame, slot, facet, Boolean.valueOf(isTemplate), values);
        checkModifiability();
        try {
            removeValuesSQL(frame, slot, facet, isTemplate);
    ...
    private Map getFrameValuesSQL(Frame frame) throws SQLException {
        if (_frameValuesText == null) {
            ...
            if (true) {
                _frameValuesText += " AND " + SLOT + " <> " + Model.SlotID.DIRECT_INSTANCES.getLocalPart();
                _frameValuesText += " AND " + SLOT + " <> " + Model.SlotID.DIRECT_SUBCLASSES.getLocalPart();
            }
            _frameValuesText += " ORDER BY " + SLOT + ", " + FACET + ", " + IS_TEMPLATE + ", " + VALUE_INDEX;
        }
    ...
    // forwarding methods
    private void setFrame(PreparedStatement stmt, int index, int projectIndex, Frame frame) throws SQLException {
        DatabaseUtils.setFrame(stmt, index, projectIndex, frame, _frameIDHelper);
    }

    private void setFrame(PreparedStatement stmt, int frameIndex, int projectIndex, int valueTypeIndex, Frame frame)
        throws SQLException {
        DatabaseUtils.setFrame(stmt, frameIndex, projectIndex, valueTypeIndex, frame, _frameIDHelper);
    }

    private void setSlot(PreparedStatement stmt, int index, int projectIndex, Slot slot) throws SQLException {
        DatabaseUtils.setSlot(stmt, index, projectIndex, slot, _frameIDHelper);
    }

    private void setFacet(PreparedStatement stmt, int index, int projectIndex, Facet facet) throws SQLException {
    ...
    private void setValue(PreparedStatement stmt, int shortIndex, int longIndex, int valueTypeIndex, Object value)
        throws SQLException {
        DatabaseUtils.setValue(
            stmt,
            shortIndex,
            longIndex,
            valueTypeIndex,
            value,
            _connection.getMaxVarcharSize(),
            _frameIDHelper);
    }
    ...
    private Frame getFrame(ResultSet rs, int frameIndex, int projectIndex, int typeIndex) throws SQLException {
        return DatabaseUtils.getFrame(rs, frameIndex, projectIndex, typeIndex, _frameFactory, _frameIDHelper);
    }
    ...
    private Object getShortValue(ResultSet rs, int index, int valueTypeIndex) throws SQLException {
        return DatabaseUtils.getShortValue(rs, index, valueTypeIndex, _frameFactory, _frameIDHelper);
    }
    ...
    private void saveFrames(KnowledgeBase kb) throws SQLException {
        ...
        if (nFrames > LOOP_SIZE) {
            Log.trace("Getting " + nFrames + " frames, please be patient, " + new Date(), this, "saveFrames");
        }
    ...
    private void printTraceMessage() {
        ...
        if (loopcount % LOOP_SIZE == 0) {
            ...
            Log.trace(text, DatabaseFrameDb.class, "printTraceMessage");
        }
    }
    ...
    private void setFrameType(PreparedStatement stmt, int index, Class frameClass) throws SQLException {
    ...
    private void nullValueError(Frame frame, Slot slot, Facet facet, boolean isTemplate) {
        Log.error("null value", this, "getFrameValuesSQL", id(frame), id(slot), id(facet), Boolean.valueOf(isTemplate));
    }
    ...
    private Map createInitialMap(Frame frame) {
        ...
        if (frame instanceof Cls) {
	        ...
	        // map.put(new Sft(directSubclassesSlot, null, false), new ArrayList());
        }
        return map;
    }
